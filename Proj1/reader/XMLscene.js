...
XMLscene.prototype.StartLights = function () {
	...
	for(var i=0; i < OmniCount; i++){
		console.log(this.graph.OmniLightsList[i]);
...
XMLscene.prototype.displayPrimitives = function (primitive, info, scale) {
    ...
    switch(info){
        case "rectangle":
            primitivecase = new MyRectangle(this, primitive[1], primitive[2], primitive[3], primitive[4]);
            if(scale) {
                var texparam = this.texArray[this.texQueue[this.texQueue.length -1]];
                primitivecase.changeTextCoords(texparam.left, texparam.right);
            }
            break;
        case "triangle":
            primitivecase = new MyTriangle(this, primitive[1], primitive[2], primitive[3], primitive[4], primitive[5], primitive[6], primitive[7], primitive[8], primitive[9]);
            if(scale) {
                var texparam = this.texArray[this.texQueue[this.texQueue.length -1]];
                primitivecase.changeTextCoords(texparam.left, texparam.right);
            }
            break;
        case "cylinder":
            primitivecase = new MyCylinder(this, primitive[1], primitive[2], primitive[3], primitive[4], primitive[5]);
            break;
        case "sphere":
            primitivecase = new MySphere(this, primitive[1], primitive[2], primitive[3]);
            break;
        case "torus":
            primitivecase = new MyTorus(this, primitive[1],primitive[2], primitive[3], primitive[4]);
            break;
    }
    primitivecase.display();
}
...
XMLscene.prototype.RecursiveSearch = function (vertex) {
    ...
    if(this.texparam != null)RecursiveSearch = ((texCheck == true || vertex.component.texture == "inherit") && (this.texparam.left != 1 || this.texparam.right != 1));
	
    for(var i = 0; i < vertex.primitives.length; i++){
        this.displayPrimitives(vertex.primitives[i],vertex.primitive_types[i],RecursiveSearch);
    }
